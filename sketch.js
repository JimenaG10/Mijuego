...
var explosion_image, explosionGroup;

function preload() {
  bird_image = loadImage("Flapp.png");
  ...
  r = loadImage("Reset.png");
  explosion_image = loadImage("Expl.png");
...
function draw() {
  ...
  if (gameState === PLAY) {
    ...
    if (bird.isTouching(pipe1Group) || bird.isTouching(pipe2Group)) {
      dieSound.play();
      explode(bird.position.x, bird.position.y);
      gameState = END;
    }
